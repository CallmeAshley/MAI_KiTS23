...
from utils import active_contour_loss
...
## If restarting
# if opt.restart:
generator.load_state_dict(torch.load('KiTS/Model_Save/2023-05-18 22:16:16.451595_DICE+AC+FOCAL_generator_140.pth'))
...
elif 'DICE+AC' in opt.loss_function:
    loss = DiceLoss(include_background=False)

elif 'DICE+AC+FOCAL' in opt.loss_function:
    loss = DiceFocalLoss(include_background = False, lambda_dice = 1.0, lambda_focal= 1.0) 
    
dice = DiceLoss(include_background=False)
...
for fold, (train_indices, val_indices) in enumerate(kf.split(dataset_indices)):
    ...
    val_subset = Subset(dataset, val_indices, is_valid=True)

    # 각 fold에 대한 DataLoader 생성
    train_loader = DataLoader(train_subset, batch_size=opt.batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_subset, batch_size=opt.val_batch_size, shuffle=False, num_workers=4)
    
    
    
    for epoch in range(opt.epoch+7, opt.n_epochs):
        ...
        for i, train_data in enumerate(train_loader):     # i  ---> batch, train_data   ----> 말 그대로 data
            ...
            if epoch <30 :
                ...
                criterion = loss(train_output, train_label) + active_contour_loss(train_output, train_label)
            ...
            DSC = 1-dice(train_output, train_label).cpu()
            

            
            Loss_train_per_epoch += criterion.item()
            Dice_train_per_epoch += DSC.item()
            
            print("[Fold %d/%d   Epoch %3d/%3d   Batch %3d/%3d] [Train loss: %.4f | DICE score: %.4f] takes %5.3f (s)."
                % (fold+1, 5, epoch+1, opt.n_epochs, i+1, len(train_loader), criterion.item(), DSC.item(), (train_end - train_start)))
            
            writer.add_scalar("Loss/train", Loss_train_per_epoch/(i+1), epoch+1)
        ...
        with torch.no_grad():
            ...
            for v, valid_data in enumerate(val_loader):
                ...
                DSCore = 1-dice(valid_output, valid_label).cpu()
                
                
                Loss_valid_ALL += cri.item()
                Dice_valid_ALL += DSCore.item()
                
                
                
                print("[Batch %3d/%3d] [Val loss: %.4f | DICE score: %.4f] takes %5.1f (s)."
                % (v+1, len(val_loader), cri.item(), DSCore.item(), (valid_end - valid_start)))
                
                
                if DSCore > best_dice:   #code  바꿔라 
                    
                    torch.save(generator.state_dict(), (opt.model_save_dir+str(dt_now)+'_'+opt.loss_function+'_generator_%d.pth') % (epoch + 1))
                    best_dice = DSCore
                
                
                writer.add_scalar("Loss/valid", Loss_valid_ALL/(v+1), epoch+1)
